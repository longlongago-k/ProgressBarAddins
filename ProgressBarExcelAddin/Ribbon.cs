...
using ProgressBarExcelAddin.Properties;
using System;
...
namespace ProgressBarExcelAddin
{
    ...
    public class Ribbon : Office.IRibbonExtensibility
    {
        ...
        {
            return GetResourceText("ProgressBarExcelAddin.Ribbon.xml");
        }
        ...
        public void OnButtonShapeProgressButton(IRibbonControl control)
        {
            ...
            foreach (Excel.Shape shp in selection)
            {
        ...
        {
            var app = Globals.ThisAddIn.Application.ActiveWindow;
            var selection = app.Selection.ShapeRange as Excel.ShapeRange;
            if(selection.Count == 0)
                return false;
            bool supported = false;
            foreach (Excel.Shape shape in selection)
            {
                supported |= isShapeSupported(shape);
            }
            return supported;
        }
        private bool isShapeSupported(Excel.Shape shape)
        {
            switch(shape.Type)
            {
